...
The web service reads `EXPO_PUBLIC_API_URL` at build time and should point to the public Render URL for `bar-par-api`. The API needs `DATABASE_URL` for Stashi and `CORS_ORIGINS` set to the final Render web origin. `JWT_SECRET` is generated by Render from the Blueprint.
